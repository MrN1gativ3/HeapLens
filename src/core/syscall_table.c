...
/* Based on Linux x86_64 arch/x86/entry/syscalls/syscall_64.tbl. */
static const char *syscall_names[500] = {
    ...
    [134] = "uselib", [135] = "personality", [136] = "ustat", [137] = "statfs", [138] = "fstatfs",
    ...
    [161] = "chroot", [162] = "sync", [163] = "acct", [164] = "settimeofday", [165] = "mount",
    ...
    [233] = "epoll_ctl", [234] = "tgkill", [235] = "utimes", [236] = "vserver", [237] = "mbind",
    ...
    [332] = "statx", [333] = "io_pgetevents", [334] = "rseq", [335] = "uretprobe",
    [336] = "uprobe",
    [424] = "pidfd_send_signal", [425] = "io_uring_setup", [426] = "io_uring_enter",
    [427] = "io_uring_register", [428] = "open_tree", [429] = "move_mount", [430] = "fsopen",
    [431] = "fsconfig", [432] = "fsmount", [433] = "fspick", [434] = "pidfd_open",
    [435] = "clone3", [436] = "close_range", [437] = "openat2", [438] = "pidfd_getfd",
    ...
    [448] = "process_mrelease", [449] = "futex_waitv", [450] = "set_mempolicy_home_node",
    [451] = "cachestat", [452] = "fchmodat2", [453] = "map_shadow_stack",
    [454] = "futex_wake", [455] = "futex_wait", [456] = "futex_requeue", [457] = "statmount",
    [458] = "listmount", [459] = "lsm_get_self_attr", [460] = "lsm_set_self_attr",
    [461] = "lsm_list_modules", [462] = "mseal", [463] = "setxattrat", [464] = "getxattrat",
    [465] = "listxattrat", [466] = "removexattrat", [467] = "open_tree_attr",
    [468] = "file_getattr", [469] = "file_setattr", [470] = "listns", [471] = "rseq_slice_yield"
};

static void heaplens_format_prot(unsigned long prot, char *buffer, size_t buffer_size) {
    size_t offset = 0;

    if (prot == PROT_NONE) {
        snprintf(buffer, buffer_size, "PROT_NONE");
        return;
    }

    if (prot & PROT_READ) {
        offset += (size_t) snprintf(buffer + offset, buffer_size - offset, "%sPROT_READ", offset == 0 ? "" : "|");
    }
    if (prot & PROT_WRITE) {
        offset += (size_t) snprintf(buffer + offset, buffer_size - offset, "%sPROT_WRITE", offset == 0 ? "" : "|");
    }
    if (prot & PROT_EXEC) {
        offset += (size_t) snprintf(buffer + offset, buffer_size - offset, "%sPROT_EXEC", offset == 0 ? "" : "|");
    }
}

static void heaplens_format_mmap_flags(unsigned long flags, char *buffer, size_t buffer_size) {
    struct {
        unsigned long flag;
        const char *name;
    } names[] = {
...
const char *heaplens_syscall_name(long syscall_number) {
    if (syscall_number < 0 || syscall_number >= (long) (sizeof(syscall_names) / sizeof(syscall_names[0]))) {
        return "unknown";
    }

    return syscall_names[syscall_number] ? syscall_names[syscall_number] : "unknown";
}
...
void heaplens_format_syscall(long syscall_number,
                             ...
                             char *buffer,
                             size_t buffer_size) {
    const char *name = heaplens_syscall_name(syscall_number);
    ...
    switch (syscall_number) {
        case 9: {
            char prot[64];
            char flags[128];
            heaplens_format_prot(arguments[2], prot, sizeof(prot));
            heaplens_format_mmap_flags(arguments[3], flags, sizeof(flags));
            snprintf(buffer,
                     buffer_size,
                     "%s(%#lx, %lu, %s, %s, %ld, %#lx) = %#lx",
                     name,
                     arguments[0],
                     arguments[1],
                     prot,
